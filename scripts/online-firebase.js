...
const fingerprint = payload => { const text = JSON.stringify({ dashboard: payload?.dashboard || {}, workspaces: payload?.workspaces || {} }); let hash = 2166136261; for (let i = 0; i < text.length; i++) hash = Math.imul(hash ^ text.charCodeAt(i), 16777619); return `${hash >>> 0}:${text.length}`; };
