...
function setStatus(kind, text) { document.querySelectorAll('.online-status').forEach(element => { element.className = `online-status ${kind}`; element.textContent = text; }); }
async function push() { if (!user || !dashboard()) return; const payload = full(); if (!payload) return; try { await setDoc(doc(db, 'teachers', user.uid, 'snapshots', 'current'), { ownerUid: user.uid, savedAt: new Date().toISOString(), payload }); await pushGuardianSnapshots(); setStatus('ok', t(teacherLocale(), 'synced')); } catch (error) { console.error(error); setStatus('err', t(teacherLocale(), 'failed')); } }
...
async function syncOnLogin() { const reference = doc(db, 'teachers', user.uid, 'snapshots', 'current'), remoteSnap = await getDoc(reference); if (remoteSnap.exists() && remoteSnap.data()?.payload) { const remote = remoteSnap.data().payload, local = full(); if (local?.exportedAt && remote.exportedAt && remote.exportedAt > local.exportedAt && confirm(teacherLocale() === 'en' ? 'A newer cloud copy exists. Use it?' : 'একটি নতুন ক্লাউড কপি আছে। সেটি ব্যবহার করবেন?')) { dashboard().setState(remote.dashboard); Object.entries(remote.workspaces || {}).forEach(([id, data]) => localStorage.setItem(workspaceKey(id), JSON.stringify(data))); } else await push(); } else await push(); stopTeacherSnapshot?.(); stopTeacherSnapshot = onSnapshot(reference, () => {}); }
async function signIn() { try { await signInWithPopup(auth, new GoogleAuthProvider()); } catch (error) { console.error(error); toast(teacherLocale() === 'en' ? 'Google sign-in could not be completed.' : 'Google সাইন-ইন সম্পন্ন করা যায়নি।', 'error'); } }
async function createRoom(studentId) {
  if (!user) return toast(teacherLocale() === 'en' ? 'Sign in as a teacher first.' : 'আগে অনলাইন শিক্ষক হিসেবে সাইন-ইন করুন。', 'error');
...
onAuthStateChanged(auth, next => { user = next; setTimeout(() => { decorate(); if (user) syncOnLogin(); guardianManager(); }, 0); });
