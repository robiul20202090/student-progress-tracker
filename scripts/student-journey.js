...
export function createStudentJourney(context) {
  ...
  const planRoadmap = (plan, student) => Array.isArray(plan?.roadmap) && plan.roadmap.length
    ? plan.roadmap
    : (student.subjects || []).map(subject => ({ id: subject.id || subject.name, name: subject.name || subject, chapters: [] }));
  ...
  async function roadmapModal(studentId) {
    ...
    const subjectItems = roadmaps.map(subject => ({ value: subject.id, label: subject.name }));
