...
export function createStudentJourney(context) {
  ...
  async function reflectionModal(studentId) {
    ...
    document.querySelector('#reflectionForm').onsubmit = async event => { event.preventDefault(); const form = new FormData(event.currentTarget); try { await saveStudentPlan(state.user.uid, studentId, { weeklyReflection: Object.fromEntries(form) }); close(); toast(state.language === 'bn' ? 'সাপ্তাহিক সারসংক্ষেপ সংরক্ষণ হয়েছে।' : 'Weekly reflection saved.'); await render(); } catch (error) { err(error); } };
  }
