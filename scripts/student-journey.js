...
export function createStudentJourney(context) {
  ...
  function reportMiniChart(day, index) {
    const vals = day.subjects.flatMap(item => [item.progress, item.homework, item.attention]).map(value => value === null || value === undefined || value === '' ? null : Number(value));
    const labels = [bn() ? 'পড়াশোনা' : 'Learning', bn() ? 'হোমওয়ার্ক' : 'Homework', bn() ? 'মনোযোগ' : 'Attention'];
    return `<article class="report-mini-chart"><h4>${esc(day.day)}</h4><div class="report-mini-legend">${labels.map(label => `<span>${esc(label)}</span>`).join('')}</div><div class="report-mini-bars">${vals.map((value, valueIndex) => `<div class="report-mini-bar"><strong>${value === null ? '—' : number(value)}</strong><i style="height:${value === null ? 3 : Math.max(4, value / 5 * 100)}%;background:${value === null ? '#e5dfcf' : scoreColor(value)}"></i><small>${valueIndex + 1}</small></div>`).join('')}</div></article>`;
  }

  function reportMonthlyChart(sheet) {
    const week = activeWeek();
    const points = (week?.rows || []).map(row => {
      const values = row.subjects.flatMap(subject => [subject.progress, subject.homework, subject.attention]).filter(value => value !== null && value !== undefined && value !== '').map(Number);
      return values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : null;
    });
    const bars = points.map((value, index) => `<div class="report-month-bar"><strong>${value === null ? '—' : value.toFixed(1)}</strong><i style="height:${value === null ? 3 : Math.max(4, value / 5 * 100)}%;background:${value === null ? '#e5dfcf' : scoreColor(value)}"></i><small>${esc((week?.rows?.[index]?.day) || '')}</small></div>`).join('');
    return `<div class="report-month-chart">${bars}</div>`;
  }

  function scoreColor(value) {
    const n = Math.max(0, Math.min(5, Number(value)));
    return `hsl(${n / 5 * 142}, 68%, 43%)`;
  }

  function weeklySheetMarkup(student) {
    const week = activeWeek();
    if (!week) return empty(bn() ? 'সপ্তাহ তৈরি করুন' : 'Create a week', '');
    const monthTabs = currentSheet.weeks.map(item => `<button type="button" class="report-week-chip ${item.id === week.id ? 'active' : ''}" data-sheet-action="switch-week" data-week-id="${esc(item.id)}">${esc(item.name)}</button>`).join('');
    const rows = week.rows.map((row, dayIndex) => row.subjects.map((subject, subjectIndex) => {
      const scoreCell = key => {
        const value = subject[key] ?? '';
        const width = value === '' || value === null ? 0 : Math.max(0, Math.min(5, Number(value))) / 5 * 100;
        return `<div class="report-score-cell"><input inputmode="numeric" type="number" min="0" max="5" step="1" value="${value ?? ''}" placeholder="0–5" data-sheet-field data-week-id="${esc(week.id)}" data-day-index="${dayIndex}" data-subject-index="${subjectIndex}" data-key="${key}" aria-label="${esc(key)}"><span class="report-score-track"><i data-fill-week="${esc(week.id)}" data-fill-day="${dayIndex}" data-fill-subject="${subjectIndex}" data-fill-key="${key}" style="width:${width}%"></i></span></div>`;
      };
      return `<tr><td class="report-day">${subjectIndex === 0 ? esc(row.day) : ''}</td><td class="report-subject">${esc(subject.name)}</td><td><input class="report-topic" type="text" value="${esc(subject.topic || '')}" placeholder="${bn() ? 'টপিক লিখুন' : 'Write topic'}" data-sheet-field data-week-id="${esc(week.id)}" data-day-index="${dayIndex}" data-subject-index="${subjectIndex}" data-key="topic"></td><td>${scoreCell('progress')}</td><td>${scoreCell('homework')}</td><td>${scoreCell('attention')}</td><td><input class="report-remark" type="text" value="${esc(subject.remark || '')}" placeholder="${bn() ? '—' : '—'}" data-sheet-field data-week-id="${esc(week.id)}" data-day-index="${dayIndex}" data-subject-index="${subjectIndex}" data-key="remark"></td></tr>`;
    }).join('')).join('');
    const dailyCharts = week.rows.map((row, index) => reportMiniChart(row, index)).join('');
    return `<section class="weekly-sheet-shell report-sheet"><div class="report-top-nav"><span>▣ ${bn() ? 'সাপ্তাহিক অগ্রগতি প্রতিবেদন' : 'Weekly progress report'}</span><button type="button" data-student-tab="checklist">✓ ${bn() ? 'চেকলিস্ট' : 'Checklist'}</button><button type="button" data-student-tab="checklist-chart">▥ ${bn() ? 'চেকলিস্ট অগ্রগতি রিপোর্ট' : 'Checklist progress report'}</button><button type="button" data-student-tab="exams">▤ ${bn() ? 'পরীক্ষার ফলাফল' : 'Assessment report'}</button></div><div class="report-controls"><div><span>${bn() ? 'মাস:' : 'Month:'}</span><button class="report-pill active" type="button">${bn() ? 'মাস ১' : 'Month 1'}</button><button class="report-outline" type="button">＋ ${bn() ? 'নতুন মাস' : 'New month'}</button><button class="report-outline" type="button">✎ ${bn() ? 'নাম বদলান' : 'Rename'}</button></div><div><span>${bn() ? 'সপ্তাহ:' : 'Week:'}</span>${monthTabs}<button class="report-outline" type="button" data-sheet-action="add-week">＋ ${bn() ? 'নতুন সপ্তাহ' : 'New week'}</button></div><button class="report-print" type="button" data-sheet-action="print">▣ ${bn() ? 'প্রিন্ট করুন' : 'Print'}</button></div><header class="report-header"><p>${bn() ? 'মাস ১ → সাপ্তাহিক অগ্রগতির প্রতিবেদন' : 'Month 1 → Weekly progress report'}</p><h2>${esc(week.name)}</h2></header><div class="report-meta"><label>${bn() ? 'শুরুর তারিখ' : 'Start date'}<input type="date" value="${esc(week.start || week.weekStart || '')}" data-week-meta="start" data-week-id="${esc(week.id)}"></label><label>${bn() ? 'শেষের তারিখ' : 'End date'}<input type="date" value="${esc(week.end || week.weekEnd || '')}" data-week-meta="end" data-week-id="${esc(week.id)}"></label><label>${bn() ? 'মোট ক্লাস হয়েছে' : 'Classes held'}<input type="number" min="0" value="${esc(week.classesHeld || '')}" data-week-meta="classesHeld" data-week-id="${esc(week.id)}"></label><div class="report-days"><span>${bn() ? 'কাজের দিন' : 'Working days'}</span><select data-sheet-action="workdays"><option value="6" ${currentSheet.workdays === 6 ? 'selected' : ''}>6</option><option value="4" ${currentSheet.workdays === 4 ? 'selected' : ''}>4</option></select></div></div><div class="report-legend"><span class="legend-low">0–1 ${bn() ? 'দুর্বল' : 'low'}</span><span class="legend-mid">2–3 ${bn() ? 'মাঝারি' : 'medium'}</span><span class="legend-high">4–5 ${bn() ? 'ভালো' : 'good'}</span><small>${bn() ? 'স্কোর (০–৫), বারটি ভরে দ্রুত বুঝুন' : 'Score 0–5; bars provide quick feedback'}</small></div><div class="report-table-scroll"><table class="weekly-entry-table report-table"><thead><tr><th>${bn() ? 'দিন' : 'Day'}</th><th>${txt('subject')}</th><th>${bn() ? 'আজকের টপিক' : 'Today’s topic'}</th><th>${bn() ? 'একাডেমিক' : 'Academic'}</th><th>${bn() ? 'হোমওয়ার্ক' : 'Homework'}</th><th>${bn() ? 'মনোযোগ' : 'Attention'}</th><th>${bn() ? 'মন্তব্য' : 'Remark'}</th></tr></thead><tbody>${rows}</tbody></table></div><div class="report-summary-cards"><div><strong data-week-average>0%</strong><span>${bn() ? 'গড় একাডেমিক অগ্রগতি' : 'Academic progress average'}</span></div><div><strong data-week-filled>0</strong><span>${bn() ? 'ভরা হোমওয়ার্ক রেকর্ড' : 'Homework records filled'}</span></div><div><strong>—</strong><span>${bn() ? 'গড় মনোযোগ' : 'Average attention'}</span></div></div><div class="report-notes"><label>${bn() ? 'এই সপ্তাহের ভালো দিক' : 'Strengths'}<textarea data-week-note="strengths" data-week-id="${esc(week.id)}" placeholder="${bn() ? 'লিখুন…' : 'Write…'}">${esc(week.notes?.strengths || '')}</textarea></label><label>${bn() ? 'দুর্বল জায়গা (পরের সপ্তাহে ফোকাস)' : 'Needs support'}<textarea data-week-note="difficulties" data-week-id="${esc(week.id)}" placeholder="${bn() ? 'লিখুন…' : 'Write…'}">${esc(week.notes?.difficulties || '')}</textarea></label><label>${bn() ? 'বক/কুইজ ফোকাস' : 'Test or quiz focus'}<textarea data-week-note="mock" data-week-id="${esc(week.id)}" placeholder="${bn() ? 'লিখুন…' : 'Write…'}">${esc(week.notes?.mock || '')}</textarea></label><label>${bn() ? 'পরের সপ্তাহের পরিকল্পনা' : 'Next week plan'}<textarea data-week-note="plan" data-week-id="${esc(week.id)}" placeholder="${bn() ? 'লিখুন…' : 'Write…'}">${esc(week.notes?.plan || '')}</textarea></label></div><section class="report-chart-section"><h3>${bn() ? 'প্রতিদিনের গ্রাফ (বিষয়ভিত্তিক তুলনা)' : 'Daily charts (subject comparison)'}</h3><div class="report-mini-grid">${dailyCharts}</div></section><section class="report-chart-section"><h3>${bn() ? 'সাপ্তাহিক মাসিক তুলনা (দিন অনুযায়ী)' : 'Weekly comparison by day'}</h3>${reportMonthlyChart(currentSheet)}</section></section>`;
  }
